    ...
    <div v-if="showPrompt"
      ...
      <div class="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl p-4 flex items-center space-x-3">
        ...
        <div class="flex-1 min-w-0">
          <div class="text-sm font-semibold text-slate-100">{{ $t('pwaInstall.title') }}</div>
          <div class="text-[11px] text-slate-400">{{ $t('pwaInstall.description') }}</div>
        </div>
        <button @click="install" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white">{{ $t('pwaInstall.install') }}</button>
